...
promptTopicSys = ("User wants to generate flowcharts and needs a topic, "
                  "give one, be concise, no more than 30 words and no "
                  "further explanations")
promptTopicHum = "Give me one topic for one flowchart"


def promptValidator(variant, threshold):
    prompt = (f"I generate a flowchart following the follow schema: "
              f"{variant}; assign to the image a number between zero and one "
              f"depending if satisfy the requirements, if the score "
              f"is greater than {threshold} then just return that number and "
              f'"Any", otherwise create a prompt for modify it such that the '
              f"image match with the schema . Just return those two anything "
              f"else! Example: 0.1 Modify the arrow from node n1 to n2 , "
              f"0.9 Any, don't use quotation marks.")
    return prompt
